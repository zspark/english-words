...
document.addEventListener("DOMContentLoaded", (e) => {
    ...
    function _switchToSection(id: string): void {
        let _nextSection: SectionBase;
        if (id === "sec-dictionary") {
            if (_currentSection == _words) return;
            _nextSection = _words;
        } else if (id === "sec-read") {
            if (_currentSection == _article) return;
            _nextSection = _article;
        } else if (id === "sec-test") {
            if (_currentSection == _test) return;
            _nextSection = _test;
        } else if (id === "sec-result") {
            if (_currentSection == _result) return;
            _nextSection = _result;
        } else if (id === "sec-setting") {
            if (_currentSection == _setting) return;
...
<p>This website is still under developing, more patience and tolerance would be much appriciated.</p>`,
    ...
    _navigator.addEventListener(NAV_EVT_SECTION, (e) => {
        const _d = (e as CustomEvent).detail;
