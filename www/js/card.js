...
class Card extends EventTarget {
    ...
    constructor(dict) {
        ...
        const ele_searchInput = this.ele_searchInput = ui.get('#id-searchInput input');
        ...
        this.ele_new_linkedWords = ui.get("#card-edit #id-new-links input");
        ...
        this.ele_btnFill = ele_action.querySelector("button[data-index='1']");
        this.ele_btnSave = ele_action.querySelector("button[data-index='2']");
        this.ele_btnDelete = ele_action.querySelector("button[data-index='3']");
        const ele_searchResult = this.ele_searchResult = ui.get('#card-searchResult');
        ele_searchResult.addEventListener("mousedown", (e) => {
            const _targetElem = this.#_selectTargetElement(e.target);
            if (!_targetElem)
                return;
    ...
    #_enterEditMode() {
        ...
        this.dispatchEvent(new CustomEvent(_b.CARD_EVT_MODE_EDIT, { detail: {} }));
    }
    #_enterReadMode() {
        if (this._currentMode === MODE_READ)
            return;
    ...
    #_save() {
        ...
        const level = this.ele_new_level.value.trim();
