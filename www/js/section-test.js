...
const _rts = cacher.localProxy.get('sec_test', {});
...
export default class SectionTest extends SectionBase {
    ...
    constructor(dict, nb, card) {
        ...
        this.ui.get("#id-form #id-toTest").addEventListener("change", (e) => {
            ...
            const _elem = ele_form.querySelector('#id-answer-form select');
            if (_elem) {
                let _s = "";
                switch (_tar.value) {
                    case "Word":
            ...
            if (ele_hmotc && ele_input_container) {
                switch (e.target.value) {
                    ...
                    case "Input":
                        ...
                        ele_input_container.replaceChildren(this.#_ele_action_input);
                        break;
                    case "Voice":
                        ele_hmotc.setAttribute("hidden", "");
        ...
        const ele_action_input = this.#_ele_action_input = ele_input_container?.querySelector("#id-test-input");
        ele_action_input?.addEventListener("keydown", (e) => {
            if (e.key !== "Enter")
                return;
            e.preventDefault();
    ...
    #_getValue(selector) {
        return this.ui.get(selector).value;
    }
    _genInfosFromQuestionaire() {
        const ops = this.ui.getAll("#id-form #id-question .bs-option input");
        const _requirement = {
            ...
            upperCase: this.ui.get("id-form #id-case").checked,
...
${JSON.stringify(_requirement, null, 4)}`);
    ...
    _startTimer() {
        const ele_action_time = this.ele_test.querySelector("#id-time");
        const ele_action_bar = this.ele_test.querySelector("#id-bar");
